...
public class Scheduler implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {// sign out of github next time
		...
		if (e.getSource() == other && tohome == true) {
		...
		while (removerun) {
			...
			for (int i = 0; i < jbs.size(); i++) {
				if (e.getSource() == jbs.get(i) && stoploop == false) {
					...
					System.out.println("removed " + i + "    size " + jbs.size() + "      ll " + ll.size()
							+ "      stoploop " + stoploop);

					jbs.get(i).setVisible(false);
					jepp.remove(jbs.get(i));
					ll.remove(i);
					stoploop = true;
					i = 9999;
					removerun = false;
			...
			if (stoploop) {
				for (int i = 0; i < jbs.size(); i++) {
					jbs.get(i).setVisible(
							false);/*
									 * System.out.println(jbs.get(i).getText());
									 * System.out.println(jbs.get(i).isVisible()); System.out.println("works2 ");
									 */
				}
		...
		if (e.getSource() == add) {// sign out of github next time
			...
			if (next == 3) {// sign out of github next time
				...
				/*
				 * String[] times = timeparse.split(":"); int hour = Integer.parseInt(times[0]);
				 * int min = Integer.parseInt(times[1]); time=(hour*60)+min;
				 */
	...
	public void advsort(String day, String date) {
		...
		if (ll.size() > 1) {
			...
			for (int i = 0; i < times.length - 1; i++) {
				try {
					testPositive(daytime, times[i]);
				} catch (SchedulingConflictException e) {
					new SchedulingConflictException().popup();
					ef = true;
				}
				if (ef) {
					break;
	...
	public static void testPositive(int daytime, int times) throws SchedulingConflictException {
		if (daytime == times) {
